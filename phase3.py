                        ...
                        if prefix == "product":
                            ...
                            for index, row in df.iterrows():
                                try:
                                    ...
                                    cursor.execute("""
                                                INSERT INTO supplier (supplier_id, supplier_name, product_id, supplies_available)
                                                    VALUES (%s, %s, %s, %s)
                                                        ON DUPLICATE KEY UPDATE
                                                                supplier_name = VALUES(supplier_name),
                                                                        supplies_available = supplies_available + VALUES(supplies_available);
                                                                        """, (supplier_id, supplier_name, product_id, supplies_available))
                                except Error as e:
                            ...
                            for index, row in df.iterrows():
                                try:
                                    ...
                                    cursor.execute("""
                                                INSERT INTO warehouse (warehouse_id, warehouse_name, supplier_id, product_id, total_availability)
                                                    VALUES (%s, %s, %s, %s, %s)
                                                        ON DUPLICATE KEY UPDATE
                                                                warehouse_name = VALUES(warehouse_name),
                                                                total_availability = total_availability + VALUES(total_availability);
                                                                        """, (warehouse_id, warehouse_name, supplier_id, product_id, int(total_availability)))
